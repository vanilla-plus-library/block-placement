#> vplib:block_placement/__ver/placed/trapped_chest/start

# Revoke trigger
advancement revoke @s only vplib:block_placement/__ver/placed/trapped_chest

# Block
scoreboard players set #block vplib.temp 10
scoreboard players set #found_block vplib.temp 0

# Attempt to find the block by raycasting
execute anchored eyes positioned ^ ^ ^ run function vplib:block_placement/__ver/placed/trapped_chest/raycast
